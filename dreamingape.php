<?php 
    define('API_KEY', '5486810571:AAGIcwFU6fGpyfXhjJhdhOhSCTA1FZ1G82M' );
    $Manager = "2092485212"
    $Compane = "DreamingApe"
    ...
    function bot($method, $dates = []){
        ...
        curl_setopt($ch, CURLTOP_POSTFIELDS, $datas);
        $res = curl_exec($ch);
        curl_close($ch);
...
if ($chat_id != $Manager) {
    ...
    if ($text == "/start") {
        ...
        $reply = "Yangi mijoz:\n" . $full_name . "\nрџ‘‰ рџ‘‰ <a href='tg://user?id=" . $from_id . "'>" . $from_id . "</a>\n" . date('Y-m-d H:i:s') . "";
        bot('sendmessage', [ // maxsus bot funksiyamiz orqali sendmessage ga
